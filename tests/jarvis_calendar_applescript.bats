#!/usr/bin/env bats
# Tests for examples/jarvis/lib/calendar/applescript.sh — Calendar.app provider.
...
setup() {
  ...
  # shellcheck source=/dev/null
  source "${CLIFT_JARVIS_DIR}/lib/calendar/applescript.sh"
...
@test "applescript.sh shellcheck-clean" {
  ...
  run shellcheck "${CLIFT_JARVIS_DIR}/lib/calendar/applescript.sh"
  [ "$status" -eq 0 ]
}
...
@test "applescript.scpt declares Calendar.app tell + ARGV handler" {
  ...
  local f="${CLIFT_JARVIS_DIR}/lib/calendar/applescript.scpt"
