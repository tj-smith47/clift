...
_seed_one_shot() {
  bash "${CLIFT_JARVIS_DIR}/cmds/remind/remind.sh" "$@" >/dev/null
}

_list() {
  bash "${CLIFT_JARVIS_DIR}/cmds/remind/remind.list.sh" "$@"
}
...
@test "--all-profiles includes reminders from every profile" {
  ...
  JARVIS_PROFILE=work bash "${CLIFT_JARVIS_DIR}/cmds/remind/remind.sh" \
