...
run_focus() {
  env -i \
    ...
    bash "$CLIFT_JARVIS_DIR/cmds/focus/focus.sh" "$@"
}
...
@test "focus interrupted by SIGTERM: end row still written" {
  setsid env -i \
    ...
    JARVIS_HOME="$JARVIS_HOME" JARVIS_PROFILE="$JARVIS_PROFILE" \
    CLI_DIR="$CLIFT_JARVIS_DIR" FRAMEWORK_DIR="$BATS_TEST_DIRNAME/.." \
    bash "$CLIFT_JARVIS_DIR/cmds/focus/focus.sh" 25m --on interrupted --silent \
    >/dev/null 2>&1 &
  local pid=$!
...
@test "focus killed with SIGKILL: orphan start, no end" {
  setsid env -i \
    ...
    JARVIS_HOME="$JARVIS_HOME" JARVIS_PROFILE="$JARVIS_PROFILE" \
    CLI_DIR="$CLIFT_JARVIS_DIR" FRAMEWORK_DIR="$BATS_TEST_DIRNAME/.." \
    bash "$CLIFT_JARVIS_DIR/cmds/focus/focus.sh" 25m --on orphan --silent \
    >/dev/null 2>&1 &
  local pid=$!
  local tries=0
  until [[ -s "$LOG" ]]; do
