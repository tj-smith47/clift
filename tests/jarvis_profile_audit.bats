...
@test "task add --profile work writes to work profile, not default" {
  ...
  FRAMEWORK_DIR="$CLIFT_FRAMEWORK_DIR" CLI_DIR="$CLIFT_JARVIS_DIR" \
    bash -c '
      set -euo pipefail
...
@test "coffee --profile home appends to home/focus.log" {
  ...
  FRAMEWORK_DIR="$CLIFT_FRAMEWORK_DIR" CLI_DIR="$CLIFT_JARVIS_DIR" \
    bash -c '
      ...
      source "$1"
    ' _ "$CLIFT_JARVIS_DIR/cmds/coffee/coffee.sh"
...
@test "focus --profile work writes start row to work/focus.log" {
  ...
  FRAMEWORK_DIR="$CLIFT_FRAMEWORK_DIR" CLI_DIR="$CLIFT_JARVIS_DIR" \
    bash -c '
      ...
      source "$1"
    ' _ "$CLIFT_JARVIS_DIR/cmds/focus/focus.sh" >/dev/null 2>&1 || true
...
@test "remind list --profile work reads work/reminders/" {
  ...
  run bash -c '
    ...
    declare -A CLIFT_FLAGS=([profile]=work)
    source "$1"
  ' _ "$CLIFT_JARVIS_DIR/cmds/remind/remind.list.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"x"* ]]
}
...
@test "note new --profile work creates note under work/notes/" {
  FRAMEWORK_DIR="$CLIFT_FRAMEWORK_DIR" CLI_DIR="$CLIFT_JARVIS_DIR" \
    bash -c '
      ...
      source "$1"
    ' _ "$CLIFT_JARVIS_DIR/cmds/note/note.new.sh"
  shopt -s nullglob
