...
@test "DIYCLI_VERSION is exported when FRAMEWORK_DIR is set" {
  run bash -c 'export FRAMEWORK_DIR="/opt/repos/task-cli"; source "$FRAMEWORK_DIR/lib/check/deps.sh"; echo "DIYCLI_VERSION=$DIYCLI_VERSION"'
  [ "$status" -eq 0 ]
  [[ "$output" == *"DIYCLI_VERSION=0.1.0"* ]]
}
