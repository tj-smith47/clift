...
write_wrapper_script() {
  if [[ $# -lt 2 ]]; then
    echo "error: wrapper_writer: write_wrapper_script requires <task-json> <dest-dir>" >&2
    return 1
  fi
  local entry="$1"
  local dest_dir="$2"
  ...
  # Wrappers live under cmds/<top>/. The filename mirrors the clift task
  # name (colons → dots) so namespaced subcommands like `db:migrate`
  # land at `cmds/db/db.migrate.sh` without colliding with the bare
  # `cmds/db/db.sh`. The router resolves `<top>:<sub>` directly to the
  # sub-named file (with a fallback to `<top>.sh`).
