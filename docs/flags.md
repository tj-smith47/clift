...
The `--task:` prefix is reserved for go-task runner-flag passthrough — `--task:watch`, `--task:dry`, `--task:interval`, etc. are wrapper-owned and never reach the parser. See [docs/cli/task-features.md](cli/task-features.md#-task-runner-flag-passthrough) for the whitelist and behavior. The bare token `watch` is also reserved at the top level as a first-token alias for `--task:watch`.
