...
| `command_post` | `lib/runtime/exec.sh` (EXIT trap) | `clift_override_command_post <default_fn> <task_name> <script_exit_code>` | Fires via EXIT trap in the script's process with the script's rc in `$3`; SIGINT delivers `130`, SIGTERM delivers `143` (explicit INT/TERM handlers stash the canonical signal rc before the EXIT trap runs — bash's default EXIT trap observes `$?=0` when a signal interrupts `source user_script` mid-run). Has access to `CLIFT_FLAGS` (the assoc array), `CLIFT_FLAG_*` env vars, and any env the script set. Cannot change the script's exit code. Per-command tier applies. |
