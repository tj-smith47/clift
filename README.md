...
Opt-in integrations are gated behind their own flags (nothing is generated by default):

```bash
clift init ~/mycli --cfgd   # also generate module.yaml for cfgd versioning
clift init ~/mycli --ci     # also generate .github/workflows/ci.yml (shellcheck)
```
